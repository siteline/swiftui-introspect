        ...
        XCTAssertViewIntrospection(of: PlatformDatePickerWithGraphicalStyle.self) { spies in
            ...
            VStack {
                DatePicker("", selection: .constant(date0))
                    ...
                    .introspect(.datePicker(style: .graphical), on: .iOS(.v14, .v15, .v16, .v17, .v18, .v26), .visionOS(.v1, .v2), customize: spy0)
                    #elseif os(macOS)
                ...
                DatePicker("", selection: .constant(date1))
                    .datePickerStyle(.graphical)
                    #if os(iOS) || os(visionOS)
                    .introspect(.datePicker(style: .graphical), on: .iOS(.v14, .v15, .v16, .v17, .v18, .v26), .visionOS(.v1, .v2), customize: spy1)
                    #elseif os(macOS)
                ...
                DatePicker("", selection: .constant(date2))
                    .datePickerStyle(.graphical)
                    #if os(iOS) || os(visionOS)
                    .introspect(.datePicker(style: .graphical), on: .iOS(.v14, .v15, .v16, .v17, .v18, .v26), .visionOS(.v1, .v2), customize: spy2)
                    #elseif os(macOS)
